...
const Orderspage = () => {
    ...
    async function getPendingOrders() {
        try {
            const response = await axiosConfig.get(`/getPendingOrdersBy/${manager_id}`);
            console.log(response.data.data);
            setOrderType(1);
            setOrderList(response.data.data);
        } catch (error) {
    ...
    async function getAcceptedOrders() {
        try {
            const response = await axiosConfig.get(`/getAcceptedOrdersBy/${manager_id}`);
            console.log(response.data.data);
            setOrderType(2);
            setOrderList(response.data.data);
        } catch (error) {
        ...
        <div>
            ...
            <div className='flex flex-col flex-wrap pl-20 pt-20'>
                ...
                <div className='flex flex-row space-x-10'>
                    ...
                    <button onClick={() => {getPendingOrders()}}>Pending orders</button>
                    <button onClick={() => {getAcceptedOrders()}}>On-going orders</button>
                ...
                <div>
                {
                    orderList.map((order) => (
                        ...
                        ? <OrderInfoContainer orderInfo={order} handelOnClick={acceptOrder}></OrderInfoContainer>
                        : (
                            order.status == 'accepted'
                            ?  <OrderInfoContainer orderInfo={order} handelOnClick={completeOrder}></OrderInfoContainer>
                            : null
                        )
                    ))
                }
